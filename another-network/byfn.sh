...
function networkUp() {
  ...
  if [ "${CERTIFICATE_AUTHORITIES}" == "true" ]; then
    ...
    export BYFN_CA1_PRIVATE_KEY=$(cd crypto-config/peerOrganizations/org1.esunego.com/ca && ls *_sk)
    export BYFN_CA2_PRIVATE_KEY=$(cd crypto-config/peerOrganizations/org2.esunego.com/ca && ls *_sk)
  fi
...
function upgradeNetwork() {
  if [[ "$IMAGETAG" == *"1.4"* ]] || [[ $IMAGETAG == "latest" ]]; then
    docker inspect -f '{{.Config.Volumes}}' orderer.esunego.com | grep -q '/var/hyperledger/production/orderer'
    if [ $? -ne 0 ]; then
    ...
    docker-compose $COMPOSE_FILES stop orderer.esunego.com
    docker cp -a orderer.esunego.com:/var/hyperledger/production/orderer $LEDGERS_BACKUP/orderer.esunego.com
    docker-compose $COMPOSE_FILES up -d --no-deps orderer.esunego.com

    for PEER in peer0.org1.esunego.com peer1.org1.esunego.com peer0.org2.esunego.com peer1.org2.esunego.com; do
...
function networkDown() {
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    # remove the docker-compose yaml file that was customized to the esunego
...
# (``peer0.org1.esunego.com`` & ``peer0.org2.esunego.com``).  Secondly, we point to
